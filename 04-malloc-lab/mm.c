...
team_t team = {
  ...
  "jofu1293@colorado.edu",
  /* Second member's full name (leave blank if none) */
  "James Wissemann",
  /* Second member's email address (leave blank if none) */
  "jawi9675@colorado.edu"
};
...
static char *nextFit_pointer; /* pointer to next block */
...
static void *coalesce(void *bp) 
{
    ...
    /* Make sure the rover isn't pointing to the free block that we just coalesced */
      if ((nextFit_pointer > (char *)bp) && (nextFit_pointer < (char *)NEXT_BLKP(bp))) 
          nextFit_pointer = bp;
    return bp;
}
...
void *mm_malloc(uint32_t size) 
{
    ...
    /* No fit was found. Allocate more memory and place the block */
