  ...
  //
{
  ...
  temp = long_int_val & 65535; // N % 4 == N & (4-1);
  if ( temp < 0 )
    {
      temp = temp + 65536;
    }
  ...
  //
{
  ...
  u_short_int_val_hi = ( unsigned short ) ( u_long_int_val >> 16 );
  u_short_int_val_lo = ( unsigned short ) ( u_long_int_val & 65535 );
  ...
  //
{
  ...
  chi = ( unsigned char ) ( u_short_int_val >> 8 );
  clo = ( unsigned char ) ( u_short_int_val & 255 );
  ...
  if ( error ) {
    ...
    for (int row = 0; row < height; row ++ ) {
      for (unsigned int col = 0; col < width; col ++ ) {
          int x = row * width + col; // one calc. per iteration vs. 3;
            image -> color[COLOR_RED  ][row][col] = rarray[x];
            image -> color[COLOR_GREEN][row][col] = garray[x];
            image -> color[COLOR_BLUE ][row][col] = barray[x];
      }
    }
    //
    //  Free the memory.
